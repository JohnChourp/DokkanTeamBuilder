...
function saveCharListTemp() {
  //set localStorage
  const char = document.getElementsByClassName("char");
  const charListTemp = Array.from(char).map((c) => ({
    name: c.getAttribute("data-char-name"),
    title: c.getAttribute("data-char-title"),
    type: c.getAttribute("data-char-type"),
    rarity: c.getAttribute("data-char-rarity"),
  ...
  localStorage.setItem("charListTemp", JSON.stringify(charListTemp));
}

function setCharListTemp() {
  let charListSavedTemp = [];
  const charListDataTemp = JSON.parse(localStorage.getItem("charListTemp"));
  ...
  for (let i = 0; i < charListDataTemp.length; i++) {
    const data = charListDataTemp[i];
    ...
    charListSavedTemp[i] = char_div;
  }

  char_container_id.innerHTML = "";
  for (let j = 0; j < charListSavedTemp.length; j++) {
    char_container_id.appendChild(charListSavedTemp[j]);
  }
}
