function updateRarity(rarityFilter, checked) {
  ...
  if (checked) {
    ...
    element.children.item(8).style.backgroundImage =
...
function addRarityAndEzaDependable(rarityFilter) {
  const ezaChecked = document
    .getElementById("eza")
    .classList.contains("checkedEzaBtn");
  const rarityChecked = document
...
function removeAllRarity() {
  ...
  const elements = filterRarity.map((rarity) =>
  ...
  for (let i = 0; i < elements.length; i++) {
    ...
    element.children[8].style.backgroundImage =
      "linear-gradient(180deg, #B6B6B6, #948D87)";
  }
}
