...
function applyFilters() {
    ...
    const dataCharCategories = "data-char-categories", dataCharRarity = "data-char-rarity",
        dataCharType = "data-char-type", dataCharClass = "data-char-class",
        dataCharAwaken = "data-char-awaken", dataCharSuperAtkType = "data-char-super-atk-type",
        dataCharEza = "data-char-eza", dataCharRecruit = "data-char-recruit",
        dataCharRelease = "data-char-release", dataCharCost = "data-char-cost",
        dataCharHp = "data-char-hp", dataCharAttack = "data-char-attack",
        dataCharDefense = "data-char-defense", dataCharName = "data-char-name",
    ...
    const filterCategories = [
        ...
        filterCategoriesNames = [
            ...
            "Earthlings", "Namekians",
            "Androids", "Artificial Life Forms",
            "Goku's Family", "Vegeta's Family",
            "Wicked Bloodline", "Youth",
            "Peppy Gals",

            "Super Saiyans", "Super Saiyan 2",
            "Super Saiyan 3", "Power Beyond Super Saiyan",
            "Fusion", "Potara", "Fused Fighters",
            "Giant Form", "Transformation Boost",
            "Power Absorption", "Kamehameha",
    ...
    const searchAfterEzaDropdown_btn = document.getElementById("search-after-eza-id");
    ...
    if (filterCategoryUsed.length > 0 && document.getElementById("full-match-category").classList.contains("checkedCategoryBtnInsidePatialFullMatch")) {
        let charListDefaultCategory = [];
        const filterCategorySet = new Set(filterCategoryUsed);

        charListDefault.forEach((char, index) => {
            const categories = char.getAttribute(dataCharCategories).split(",");
            const categorySet = new Set(categories);

            let match = true;
            for (const filterCategory of filterCategorySet) {
                if (!categorySet.has(filterCategory)) {
                    match = false;
                    break;
                }
            }
            ...
            if (match) {
                charListDefaultCategory[index] = char;
            }
        });
    ...
    for (let i = 0; i < searchOneCharDropdownValue.length; i++) {
        if (searchOneCharDropdownValue.item(i).classList.contains("checkedSearchOneCharBtn")) {
            charListDefault = addSearchOneCharDropdownClass(i, charListDefault);
        }
    }
